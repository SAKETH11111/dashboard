...
import type { Contaminant, WaterSeriesResponse } from "@/types/water"
...
const DASHBOARD_CARDS: DashboardCard[] = [
  { contaminant: "nitrate", title: "Nitrate", description: "Monthly running average compared to 10 mg/L MCL." },
  { contaminant: "ecoli", title: "E. coli", description: "Weekly monitoring across public beaches and utilities." },
  { contaminant: "pfas", title: "PFAS (PFOA + PFOS)", description: "Highest detections benchmarked to the new 4 ppt MCL." },
  ...
  { contaminant: "fluoride", title: "Fluoride", description: "Monthly results relative to EPA 2 mg/L secondary standard." },
]
...
function formatLatest(series: WaterSeriesResponse) {
  ...
  const units = series.unit
  const value = latest.value

  let formatted: string
  if (value >= 100) {
    formatted = `${value.toFixed(0)} ${units}`
  } else if (value >= 10) {
    formatted = `${value.toFixed(1)} ${units}`
  } else if (value >= 1) {
    formatted = `${value.toFixed(2)} ${units}`
  } else {
    formatted = `${value.toFixed(3)} ${units}`
  }
...
function computeDelta(series: WaterSeriesResponse) {
  ...
  const sorted = [...series.points].filter((pt) => typeof pt.value === "number") as Array<
  ...
  return {
    ...
    formatted:
      delta === 0
        ? "No change"
        : `${delta > 0 ? "+" : "−"}${Math.abs(delta).toFixed(delta >= 1 || delta <= -1 ? 1 : 2)} ${series.unit}`,
...
export async function WaterCards() {
  ...
  const byContaminant = new Map<Contaminant, WaterSeriesResponse>()
            ...
            <Card key={definition.contaminant} className="border-dashed border-border/50">
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  No cached data yet. Run `npm run water:etl` to build initial JSON fixtures.
                </p>
              </CardContent>
            </Card>
          )
