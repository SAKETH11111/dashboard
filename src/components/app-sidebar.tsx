...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  const { preferences, setPreferences, updatePreferences } = useUserPreferences()
  ...
  const handleAddList = React.useCallback(() => {
    ...
    const name =
      typeof window !== "undefined" ? window.prompt("Name your list", defaultName) ?? "" : defaultName
    ...
    setPreferences((current) => ({
      ...current,
      lists: [...current.lists, { id: newListId, name: trimmed, datasetIds: [] }],
  ...
  const handleRenameList = React.useCallback(
    (listId: string, currentName: string) => {
      const nextName =
        typeof window !== "undefined" ? window.prompt("Rename list", currentName) ?? "" : currentName
  ...
  const handleRemoveList = React.useCallback(
    (listId: string) => {
      if (
        ...
        !window.confirm("Remove this list? Datasets will remain available in the catalog.")
      ) {
        return
      }
      ...
      setPreferences((current) => {
        ...
        if (remaining.length === 0) {
          ...
          return {
            ...current,
            lists: [{ id: fallbackId, name: "My Saved Datasets", datasetIds: [] }],
            activeListId: fallbackId,
          }
        }

        const nextActiveId = current.activeListId === listId ? remaining[0]?.id ?? null : current.activeListId
      ...
      <SidebarContent>
        ...
        <SidebarGroup>
          ...
          <SidebarMenu>
            {lists.map((list) => {
              ...
              return (
                <SidebarMenuItem key={list.id}>
                  ...
                  {datasetCount ? <SidebarMenuBadge>{datasetCount}</SidebarMenuBadge> : null}
                  <SidebarMenuAction
                    type="button"
            ...
            <SidebarMenuItem>
              <SidebarMenuButton type="button" onClick={handleAddList} className="text-muted-foreground">
